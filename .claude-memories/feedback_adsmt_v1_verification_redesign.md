...
**2026-06-01 본격 진입 (P-redesign.1 sign-off)**: R4=(b') / R6=(b')
사용자 결정으로 hold pattern **사실상 해제**:
- 본격 작업 (proof body 작성 / cross-validation 실험 / 도구 본격 구현
  / 재설계 cycle 진입) 모두 즉시 OK
- P-redesign.8 (spec 일괄 마킹) 만 stable release 후 deferred ← 곧 정정
- R1=(a') 의 Lean4 제외 (adsmt 측 Lean4/OxiLean blocker) — Y4 측에서
  도 Lean4 backend 측은 후속 cycle 로 deferred
- 본격 ledger: `.claude-notes/trackers/adsmt-integration-tracker.md`
  §10 (P-redesign.1)

**2026-06-01 완전 해제 (P-redesign.8 도 즉시 trigger, 사용자 결정)**:
- **모든 P-redesign sub-cycle 이 stable release 무관하게 즉시 진입
  가능** — P-redesign.8 (Y4 spec v1.x patch 일괄 마킹) 도 포함
- 근거: Y4 측 channel pin 이 testing (rolling, `feedback_adsmt_testing_
  channel_pin.md`) 이라 stable release timing 과 spec 마킹 timing 의
  정합 무의미
- spec 마킹은 testing channel 기준 — rolling 후속 갱신은 v1.x patch 로
- channel 옵션 (i)/(ii)/(iii) 중 (i) testing 계속 채택 (stable 전환 X,
  hybrid X)
- 이전 "hold pattern" 표현은 historical record 로만 보존 — 실제 hold
  영역 0
