...
| **P-redesign.4** | Rocq theory 3 (Y4.Lease.Spec + Y4.IPC.Refinement + Y4.Sel4.Wrapper) + adsmt-emit-rocq 통합 spec | P-redesign.2 ✅ | ✅ **2026-06-01 완료** (`docs/verus_to_rocq.md` 신설; `proofs/coq/README.md` 갱신 — 3 theory bottom-up 순서 R4.1 + Ltac2-only R4.5 + nested directory naming R4.6 + adsmt-emit-rocq cargo git dep R4.2; `~/y4-verus2rocq/` sibling repo scaffold 신설 대기 (R4.2=b); cluster 별 rolling emission R4.7) |
| **P-redesign.5** | `y4-verus2isabelle` 의 adsmt-emit-isabelle wrapper 재정의 (P3.6 §3.2 갱신, Lean4 제외) | P-redesign.2 ✅ | ✅ **2026-06-01 완료** (`verus_to_isabelle.md` §3.2 rewrite R5.1=b — adsmt-emit-isabelle wrapper `src/emitter/adsmt_wrap.rs` + cargo git dep + Lean4 backend 제외 + verus-pin.toml/isabelle-pin.toml 삭제 — unified-toolkit-pin.toml 정합; §1.3 + §1.5 Lean4 제외 명시 R5.3=d; theory file naming `Y4_<Domain>_<Module>.thy` 유지 R5.4; cluster 별 rolling R5.5; `~/y4-verus2isabelle/` sibling repo scaffold 신설 대기 R5.6=a) |
