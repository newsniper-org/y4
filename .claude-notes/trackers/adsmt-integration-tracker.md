...
| 2026-06-01 (P-redesign.2/.3/.4/.5/.6 sign-off) | unified-toolkit-pin.toml + AV proof body 4-cluster plan + Rocq/Isabelle sibling repos + SMT cross-validation tracker | Y4 측 spec/tracker/scaffold land | §10.3 |
| 2026-06-03 (RC.6-1 tag) | **adsmt `v1.0.0-rc.6-1` tag** = `2c46803` "fix .gitmodules".  baseline `e878a14` 이후 2 commit (oxiz submodule branch `0.2.2` → `0.2.3-feat/enable-writer` + commit pin `8d2ec3f` → `1297944`).  대규모 변경 X — OxiZ branch naming fix 만 | unified-toolkit-pin.toml `[oxiz]` 와 정합 (이미 갱신, P-redesign.2 산출물 정합).  `.lock` baseline adsmt commit `450b986` → `2c46803` 갱신 | §10.6 |
| **2026-06-03 (R7 sign-off)** | **R7.1~R7.12 sign-off — Y4-seL4 integration plan.**  sibling repo (`~/y4-verus2rocq/` + `~/y4-verus2isabelle/`) 폐기 (P-redesign.4/.5 superseded), Verus fork (`<Y4>/verus-fork/`, R3.11+R3.12+R7.3 scope 확장 — `-V emit-isabelle` / `-V emit-rocq` / AOT prelude bank / JIT trace load wire) 가 single source of truth.  Y4 측 작업 = `proofs/verus/justfile` consumer pattern adapt + `proofs/isabelle/` 신설.  L4.verified inbound = 사용자 manual | unified-toolkit-pin.lock baseline rc.29 갱신 (adsmt = `03f33a9`, adsmt-contrib = `33349dc`, sound + complete 모두 확보) | §10.3 (P-redesign.4/.5 superseded mark), y4-sel4-integration-tracker (신설) |
| **2026-06-03 (PR-Verus-Backend land)** | **verus-fork backend-pluggable HEAD 가 R3.11+R3.12+R7.3 모든 patch + AOT + JIT + emit recipe land 완료** (P-vb.1~P-vb.12 모두 ✅).  주요 commit: `8a635f53a` (rc.29 Tseitin) + `c1b067359` (rc.28 AOT sound) + `5533adfe4` (AOT prelude bank script) + `cd86e9b81` (consumer/justfile template) | Y4 측 R7.11/R7.12 milestone 즉시 진입 가능 — `cd proofs/verus && just verify-adsmt && just emit-isabelle && just cross-check` | pr-verus-backend-tracker §3 (시작 조건 모두 ✅) + §4 (모든 phase ✅) + y4-sel4-integration §3 milestone 1 ✅ |
| **2026-06-08 (rc.30 — Verus interop complete)** | adsmt `v1.0.0-rc.30` (`e2951a8`) — lu-smt 의 declare-datatypes parameterized + bit-vectors + let + indexed-identifier + canonical reason-unknown + OxiZ delegation (`ADSMT_OXIZ_PATH` opt-in, 또는 PKGBUILD in-process build).  Y4 측 declare-datatypes block (.local-requests-to/adsmt/2026-06-04) resolve.  `just verify-adsmt` = `54 verified, 0 errors` (Z3 backend 와 동일) | `.lock` baseline e2951a8 갱신 + `proofs/verus/justfile` 의 `_lu_smt` default = `/usr/bin/lu-smt` (system PKGBUILD 우선) | y4-sel4-integration §1 R7.11 verify-adsmt ✅ + §2 Cluster 1 AV1 row + §7 #1 close |
| **2026-06-08 (AOT ✅ + JIT v0 stub)** | `just verify-adsmt-fast` (AOT prelude bank, `scripts/aot-bake-prelude.sh`) = `54 verified, 0 errors` ✅ + bank generated.  JIT 측은 v0 stub (lu-smt `--jit-trace-{emit,load}` flag ✅ §3.5.G, 단 replay machinery §3.5.F 미land + Verus 측 `-V jit-trace-load` flag 부재) | AOT functional 검증 완료 — 후속 cluster 의 verify cost ↓.  JIT 은 §3.5.F land 후 + verus 측 patch 후 activation 가능 | pr-verus-backend-tracker §1.6 + §4 P-vb.11 ✅ / P-vb.12 ⚠️ |
| **2026-06-08 (rc.32 — emit-cert + WASM ecosystem)** | adsmt `v1.0.0-rc.32` — `lu-smt --emit-cert{,-dir,-format}` flag land + WASM emitter ecosystem 신설 (`adsmt-emit-{contract,pm,runtime,cli}` + `adsmt-env`).  contrib `isabelle.adsmt-emit` / `rocq.adsmt-emit` package + native bin path (`rocq` / `isabelle` 이름) | Y4 측 emit pipeline 의 mechanism 확정 — `--emit-cert-dir` env hook + WASM emitter manifest | reply `2026-06-08-emit-cert-resolved-plus-wasm-emitter-redesign.md` |
| **2026-06-09 (rc.32.1 — adsmt-emit packaging)** | `adsmt-cli-testing` PKGBUILD 가 `adsmt-emit` + `adsmt-env` binary 추가 install (`/usr/bin/`).  cert wire 변경 0 | 사용자 system 측 `adsmt-emit` CLI 즉시 사용 가능 | reply `2026-06-09-rc32.1-emit-toolchain-now-arch-packaged.md` |
| **2026-06-09 (rc.33 — R7.11 end-to-end unblock)** | adsmt `v1.0.0-rc.33` (`5a1a6ee`) + emit-runtime fix `c043287` + OxiZ `0.2.4-feat/streaming-stdin`.  Gap A (delegated-unsat cert via `build_delegated_unsat_cert`) + Gap B (flat hash-cons serde, 6.8MB→1.0MB) + B' (wasmi stack ceiling).  추가 soundness fix (rc.32 theory atom + OxiZ 0.2.4 simplex `pop` restore) | Y4 측 R7.11 end-to-end pipeline ✅ — verus-fork `22e10f196` 가 `-V emit-{isabelle,rocq}` wire + ADSMT_CERT_DIR hook 정합 | reply `2026-06-09-cert-emit-pipeline-end-to-end-rc33-path-to-R7.11.md` + verus-fork `2026-06-09-R7.11-unblocked-full-emit-pipeline-end-to-end.md` |
| (대기) | adsmt v1.0.0 stable release | rolling 따라가는 testing channel 기준 — Y4 측 `[adsmt]` + `[adsmt-contrib]` 모두 stable-v1 channel 로 전환 예정 (R7.6, 사용자 결정) | §10.6 |
...
| **P-redesign.3** | AV1~AV40 proof body 의 sub-cluster 별 작성 plan (4 cluster 순서 결정 + 분량 추정) | P-redesign.2 ✅ | ✅ **2026-06-01 완료** (`.claude-notes/trackers/av-proof-body-tracker.md` 신설; R3.1~R3.12 sign-off — 4 cluster boundary + 순서 (amdv lower → amdv upper → power upper → power lower) + 분량 ~9400 LoC + file layout natural grouping + 의존 graph DAG + cross-validate timing batch-per-cluster + PR-N scope 매핑 (PR-2 amdv 2 cluster + PR-5d power 2 cluster) + Lean4 retrofit watch + **R3.11 Verus 본체 patch 별도 sub-PR** (z3 + OxiZ + adsmt 3 backend trait 통일) + **R3.12 opt-in 3-way** (adsmt third backend, abductive verdict reporter, AV5/12/15/23/24/30 6 invariant 후보)) |
| **P-redesign.4** | Rocq theory 3 (Y4.Lease.Spec + Y4.IPC.Refinement + Y4.Sel4.Wrapper) + adsmt-emit-rocq 통합 spec | P-redesign.2 ✅ | ⚠️ **superseded by R7 sign-off (2026-06-03)** — sibling repo (`~/y4-verus2rocq/`) wrapper 가정이 wrong.  Verus fork (R7.3) 가 `-V emit-rocq` 자동 emit, `adsmt-emit-rocq` CLI 가 cert→`.v` 직접 변환.  R4.1 의 manual 3 theory plan + R4.5 Ltac2-only + R4.6 nested naming 은 유지 (Verus 표현 못하는 high-level theory), `~/y4-verus2rocq/` sibling 도구는 폐기 |
| **P-redesign.5** | `y4-verus2isabelle` 의 adsmt-emit-isabelle wrapper 재정의 (P3.6 §3.2 갱신, Lean4 제외) | P-redesign.2 ✅ | ⚠️ **superseded by R7 sign-off (2026-06-03)** — sibling repo (`~/y4-verus2isabelle/`) wrapper 가정이 wrong.  Verus fork (R7.3) 가 `-V emit-isabelle` 자동 emit, `adsmt-emit-isabelle` CLI 가 cert→`.thy` 직접 변환.  R5.3 Lean4 제외 + R5.4 theory file naming `Y4_<Domain>_<Module>.thy` 는 유지 (R7.7 정합), `~/y4-verus2isabelle/` sibling 도구는 폐기 |
