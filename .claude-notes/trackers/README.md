...
| `pr-verus-backend-tracker.md` | Verus 본체 patch (R3.11 + R3.12 의 산출물) — `--backend=z3|oxiz|adsmt` flag + abductive verdict reporter.  본 작업은 **별도 Claude Code 세션** 에서 진행 (`~/verus-fork`, Y4 의 sibling repo 패턴).  9 phase (P-vb.1~P-vb.9), 분량 ~1100 LoC | **2026-06-01 (active, preparation)** | `av-proof-body-tracker.md` §1 R3.11 + R3.12 + §7 + `smt-cross-validation-tracker.md` §9 + `verus_to_isabelle.md` §3.6 + `unified-toolkit-pin.toml` `[verus]` |
