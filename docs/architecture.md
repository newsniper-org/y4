...
이 거버넌스 세부사항은 외부 첫 기여자 발생 시점에 정리. 현 단계 (Phase A–C) 는 내부 개발만 — DCO sign-off 채택 (CONTRIBUTING.md §2), CLA 결정은 첫 외부 기여자 발생 시점.
...
> **⚠ 본 §의 Phase 0–4 모델은 historical.** Y4 가 WaveTensor RTL 의
> 실제 운용 전에 먼저 개발된다는 사용자 결정 (2026-05-04) 으로
> **Phase A–E 모델 (`docs/phase_plan.md`) 로 대체**되었다. 본 섹션은
> 원본 디자인 메모의 사고 과정을 보존하기 위해 그대로 둔다 — 현재
> 진행 상황과 phase 트리거는 `docs/phase_plan.md` 가 정전.

```
Phase 0  Linux + Rust daemon              ← (대체됨)
            |
            v   (PoC + 가속기 RTL 안정화 후)
...
> **⚠ 본 §도 historical.** "RTL 안정화 후 hypervisor" 라는 직선
> 의존을 가정하지만, 현 Phase A–E 모델 (`docs/phase_plan.md`) 은
> hypervisor 를 RTL 운용 전에 mock 위에서 먼저 개발한다. 실제 진행
> 상황은 `docs/phase_plan.md` 와 `README.md` 의 "Subsystem status" 표
> 참조.
...
> **⚠ 본 §의 트리거 목록도 historical** — 원안 (RTL 안정 후 hypervisor)
> 의 가정 위. 현 트리거는 Phase A → B (HIU ABI 동결), Phase B → C
> (HIU 통합 진입), Phase D → E 등으로 `docs/phase_plan.md` 가 정의.
> 본 historical 목록은 원본 사고를 보존하기 위해 그대로 둔다.
