...
### 1.7 seL4 팀 inbound contract (G — R7.10 갱신, 2026-06-03)

도구 결과를 받은 seL4 팀의 사용 흐름:

1. Y4 PR-4 (`amdv_safety.md` §6.2) 가 `proofs/isabelle/` 산출물 첨부 +
   `unified-toolkit-pin.lock` (R7.6) + `verus-fork/` submodule commit
   pin (gitlink) 모두 첨부.  artifact = `theory Y4_AmdvSafety imports
   Main begin ... end` (R7.7 의 2-layer imports chain + cluster sub-
   grouping)
2. seL4 팀이 자체 Isabelle 환경의 ROOTS 에 `<Y4>/proofs/isabelle/` 추가
   — `imports Main` 만 사용하므로 추가 sessions / AFP entry / l4v 측
   theory 의 dep 0
3. sorry 채움 (Isar 자체 증명) 또는 axiom 수용 결정 — Y4 측 mode 표시
   는 generated `.thy` 의 주석에 명시 (T-i / T-ii / T-iv)
4. (T-iv) SMT-LIB replay 결과 fail 시 `adsmt-emit-isabelle` 재실행 또는
   manual Isar 완성
5. import path / theory 이름 변경 = v2 (incompatible) — 기존 use 무영향
   추가는 v1.x patch
6. trust ledger (R7.10): (T-i) seL4 책임 / (T-ii) Y4 책임 / (T-iv)
   Isabelle stdlib 책임

**Trust 가능 조건 (R7.10 추가, 2026-06-03 모두 충족 ✅)**:
- PR-Verus-Backend (R3.11+R3.12+R7.3) land ✅ — verus-fork backend-
  pluggable HEAD 가 `-V adsmt` + `-V emit-isabelle` 모두 ship (pr-verus-
  backend-tracker §4 P-vb.1~P-vb.12 ✅)
- adsmt rc.28+ + rc.29+ 도달 ✅ — sound (all paths) + complete (OR-of-
  AND via Tseitin) 둘 다 확보 (R7.6 정합)
- 본 `unified-toolkit-pin.lock` baseline commit hash 가 위 두 조건 만족
- 2026-06-03 baseline (adsmt = `03f33a9` rc.29 + adsmt-contrib =
  `33349dc` rc.28) 가 양 조건 만족 ✅ — Y4 측 즉시 inbound contract 활성
...
**산출물 위치**: `<Y4>/proofs/isabelle/` (R7.7) — `proofs/isabelle/README.md`
가 본 inbound contract 의 entry point.
...
> **R7 sign-off supersede note (2026-06-03)**: §3.1 ~ §3.5 + §3.7 ~
> §3.8 의 sibling repo (`y4-verus2isabelle`) 측 도구 형상 본문은 모두
> **superseded** — Verus fork (PR-Verus-Backend, R3.11+R3.12+R7.3 의
> scope) 가 `-V adsmt` flag + `-V emit-isabelle` / `-V emit-rocq` +
> AOT prelude bank + JIT trace load 모두 직접 wire 하므로 Y4 측 wrapper
> 도구 의미 0.  새 model = consumer/justfile 패턴을 Y4 측 `proofs/verus/
> justfile` 로 adapt (R7.2) + `adsmt-emit-isabelle` / `adsmt-emit-rocq`
> CLI binary 호출 (adsmt-contrib testing branch pin, R7.6).  Y4 측 별도
> sibling repo 신설 X.
>
> **§3.6 (unified-toolkit-pin) 만 유효** — P-redesign.2 + R7.6 의
> 산출물.
>
> 자세한 설계 ledger: `~/.claude/plans/jazzy-gliding-puppy.md` R7.1~R7.12
> + `.claude-notes/trackers/y4-sel4-integration-tracker.md` (신설).
...
### 3.2 (A) 채택 시 내부 구조 (P-redesign.5 갱신, 2026-06-01 — superseded R7 2026-06-03)

> **R7 supersede**: 본 §3.2 의 sibling repo 측 layout (`y4-verus2isabelle`)
> 본문은 superseded — 위 §3 introduction supersede note 참조.  새 model
> = Verus fork 의 `-V emit-isabelle` flag 자동 emit + `adsmt-emit-isabelle`
> CLI binary 호출.  Y4 측 `proofs/verus/justfile` 의 `emit-isabelle`
> recipe (R7.2) + `.thy` 산출물 `<Y4>/proofs/isabelle/` (R7.7).
>
> 본문은 historical reference 로 보존:
