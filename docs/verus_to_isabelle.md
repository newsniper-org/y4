...
**Trust 가능 조건 (R7.10 추가)**:
- PR-Verus-Backend (R3.11+R3.12+R7.3) land 후 — Verus fork 의 `-V adsmt`
  + `-V emit-isabelle` flag 정합 확인
- adsmt rc.28+ + rc.29+ 도달 후 — sound (all paths) + complete (OR-of-
  AND via Tseitin) 둘 다 확보 (R7.6 정합)
- 본 `unified-toolkit-pin.lock` baseline commit hash 가 위 두 조건 만족
- 2026-06-03 현재 baseline (adsmt = `03f33a9` rc.29 + adsmt-contrib =
  `33349dc` rc.28) 가 양 조건 만족 ✅ — PR-Verus-Backend land 만 남음
