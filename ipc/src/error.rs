...
//! Mirror of `proofs/verus/src/error.rs` — duplicated here pending a
//! shared `y4-error` crate that lands with `kernel/`.  See
//! `alloc/src/error.rs` for the parallel comment.
...
pub enum Y4Error {
    ...
    Timeout,
    /// Caller violated a precondition (bad layout, double-free, ...).
    InvalidArg,
